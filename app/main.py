...
def leaderboard():
    ...
    if _leaderboard_cache['expires_at'] > current_time and _leaderboard_cache['data'] is not None:
        scores = _leaderboard_cache['data']
    else:
        # Slow path: acquire lock and double-check
        with _leaderboard_lock:
            current_time_locked = time.time()
            if _leaderboard_cache['expires_at'] > current_time_locked and _leaderboard_cache['data'] is not None:
                scores = _leaderboard_cache['data']
            else:
                try:
                    ...
                    _leaderboard_cache['data'] = new_scores
                    _leaderboard_cache['expires_at'] = current_time_locked + CACHE_TTL
                    scores = new_scores
